...
from moonshot.models import RunResult, is_win_result
...
def _apply_params(bp: dict) -> None:
    ...
    print(f"  ✏️  Updated strategy.py with best params.")

def optimize(n_trials: int = 50, start: Optional[datetime] = None, end: Optional[datetime] = None):
    if start is None: start = datetime(2025, 1, 1, tzinfo=timezone.utc)
