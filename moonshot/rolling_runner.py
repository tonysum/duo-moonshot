"""RollingRunner — backtesting loop for Moonshot-R24 strategy.

Key difference from MoonshotRunner:
  - Signals are scanned EVERY HOUR (not just at midnight)
  - Uses preload_hourly_gainers (K1h-based 24h rolling change)
  - Delay is in hours (not days)
  - Cooldown tracking prevents duplicate signals for same symbol
"""
...
class RollingRunner:
    """Orchestrates the backtesting loop for the Moonshot-R24 strategy."""

    def __init__(self, feed: RollingDataFeed, account: Account, strategy: RollingStrategy, verbose: bool = True):
        self._feed = feed
    ...
    def run(self, start: datetime, end: datetime) -> RunResult:
        ...
        # Cooldown tracker: {symbol: last_signal_datetime}
        cooldown_tracker: dict[str, datetime] = {}

        # Signal log
        realtime_csv = Path("reports") / f"rolling_signals_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        ...
        hourly_gainers = self._feed.preload_hourly_gainers(
            start, end, top_n=cfg.top_n, window_hours=cfg.rolling_window_hours,
        )

        current_dt = start
        while current_dt <= end:
            # 1. Exit checks (5m granularity)
            still_open = []
            ...
            # 2. Timeout (daily check at 23:00)
            ...
            for item in pending_entries:
                ...
                if entry_p:
                    ...
                    if 0 < invest <= cap and remaining_slots > 0:
                        ...
                        if self.verbose:
                            logger.info("  🎯 R24 %s @ %.4f invest=%.0f", symbol, entry_p, invest)
                else:
                    still_pending.append(item)
        ...
        if self.verbose:
            logger.info(
                "[R24] done. %d trades, capital: %.0f → %.0f",
    ...
    # ------------------------------------------------------------------
    # Result builder (identical to MoonshotRunner)
    # ------------------------------------------------------------------
    ...
    def export_csv(
        ...
        """Write R24 trade results to CSV."""
